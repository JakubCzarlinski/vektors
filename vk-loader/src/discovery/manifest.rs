...
pub(super) struct RawLayer<'a> {
    ...
    instance_extensions: Option<&'a Value>,
    device_extensions: Option<&'a Value>,
    enable_environment: Option<&'a Value>,
    disable_environment: Option<&'a Value>,
    component_layers: Option<&'a Value>,
    blacklisted_layers: Option<&'a Value>,
    override_paths: Option<&'a Value>,
    app_keys: Option<&'a Value>,
    functions: Option<&'a Value>,
    pre_instance_functions: Option<&'a Value>,
}
impl<'a> RawLayer<'a> {
    pub(super) fn from_value(value: &'a Value) -> Option<Self> {
        ...
        Some(Self {
            ...
            layer_type: value.field("type").and_then(Value::as_bytes),
            library_path: value.field("library_path").and_then(Value::as_bytes),
            api_version: value.field("api_version").and_then(Value::as_bytes),
            library_arch: value.field("library_arch").and_then(Value::as_bytes),
            implementation_version: value
                .field("implementation_version")
                .and_then(Value::as_bytes),
            description: value.field("description").and_then(Value::as_bytes),
            instance_extensions: value.field("instance_extensions"),
            device_extensions: value.field("device_extensions"),
            enable_environment: value.field("enable_environment"),
            disable_environment: value.field("disable_environment"),
            component_layers: value.field("component_layers"),
            blacklisted_layers: value.field("blacklisted_layers"),
            override_paths: value.field("override_paths"),
            app_keys: value.field("app_keys"),
            functions: value.field("functions"),
            pre_instance_functions: value.field("pre_instance_functions"),
        })
    }
}
    ...
    fn from_value(value: &'a Value) -> Option<Self> {
        ...
        Some(Self {
            ...
            layer: match value.field("layer") {
                Some(value) => Some(RawLayer::from_value(value)?),
                None => None,
            },
            layers,
        })
...
pub(super) fn printed_bytes_fit(value: &[u8], capacity: usize) -> bool {
    // The upstream cJSON fork sizes the preallocated print using the escaped
    // representation even though its loader-specific printer ultimately
    // writes the decoded string. Preserve that rejection boundary.
    let printed_length = value.iter().fold(0_usize, |length, byte| {
    ...
    printed_length < capacity
}

fn raw_c_string_array(value: Option<&Value>) -> Box<[CString]> {
...
pub(super) fn parse_raw_layer(
    ...
    let pre_instance = (implicit && supports_pre_instance)
        ...
        .flatten()
        .and_then(RawLayerFunctions::from_value);
    let api_version = parse_api_version_bytes(layer.api_version.take())?;
    ...
    let library_arch = layer.library_arch.take();
    ...
    Some(LayerManifest {
        ...
        implementation_version: parse_manifest_u32(layer.implementation_version.take()?),
...
pub(super) fn parse_layer_manifest_inner(
    ...
    if shadow_allocations {
        match shadow_layer_json_allocations(path, &bytes) {
    ...
    if let Some(layers) = root.layers {
        ...
        let manifests: Box<[_]> = collect_values(layers.iter().enumerate().filter_map(
            |(source_index, layer)| {
                parse_raw_layer(
                    path,
                    RawLayer::from_value(layer)?,
                    source_index,
                    implicit,
    ...
    let manifests: Box<[_]> = collect_values(root.layer.and_then(|layer| {
        parse_raw_layer(
            path,
            layer,
            0,
...
pub(crate) fn parse_layer_manifest(path: &Path, implicit: bool) -> Box<[LayerManifest]> {
    parse_layer_manifest_inner(path, implicit, true).0
}

pub(crate) fn reparse_layer_manifest(path: &Path, implicit: bool) -> Box<[LayerManifest]> {
    parse_layer_manifest_inner(path, implicit, false).0
}
