...
use crate::commands::PFN_vkDestroyPrivateDataSlot;
...
impl<'dev> Drop for PrivateDataSlot<'dev> {
  fn drop(&mut self) {
    ...
    unsafe {
      ((&self.parent.private_data_slot_table).vkDestroyPrivateDataSlot).unwrap_unchecked()(
        self.parent.raw(),
        self.raw,
        core::ptr::null(),
      )
...
impl<'dev> PrivateDataSlot<'dev> {
  ...
  pub const fn table(&self) -> &PrivateDataSlotDispatchTable {
    &self.parent.private_data_slot_table
  }
