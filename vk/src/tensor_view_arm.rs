...
use crate::commands::PFN_vkDestroyTensorViewARM;
...
impl<'dev> Drop for TensorViewARM<'dev> {
  fn drop(&mut self) {
    ...
    unsafe {
      ((&self.parent.tensor_view_arm_table).vkDestroyTensorViewARM).unwrap_unchecked()(
        self.parent.raw(),
        self.raw,
        core::ptr::null(),
      )
...
impl<'dev> TensorViewARM<'dev> {
  ...
  pub const fn table(&self) -> &TensorViewARMDispatchTable {
    &self.parent.tensor_view_arm_table
  }
