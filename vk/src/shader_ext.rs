...
use crate::commands::PFN_vkDestroyShaderEXT;
#[cfg(feature = "VK_EXT_shader_object")]
use crate::commands::PFN_vkGetShaderBinaryDataEXT;
...
impl ShaderEXTDispatchTable {
    ...
    F: Fn(*const c_char) -> Option<unsafe extern "system" fn()>,
  {
    Self {
      #[cfg(feature = "VK_EXT_shader_object")]
      vkDestroyShaderEXT: loader(c"vkDestroyShaderEXT".as_ptr())
        .map(|f| unsafe { core::mem::transmute(f) }),
      #[cfg(feature = "VK_EXT_shader_object")]
      vkGetShaderBinaryDataEXT: loader(c"vkGetShaderBinaryDataEXT".as_ptr())
        .map(|f| unsafe { core::mem::transmute(f) }),
    }
  }
}
