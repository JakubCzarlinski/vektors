...
use crate::commands::PFN_vkDestroySamplerYcbcrConversion;
...
impl<'dev> Drop for SamplerYcbcrConversion<'dev> {
  fn drop(&mut self) {
    if self.raw == VkSamplerYcbcrConversion::NULL {
      return;
    }
    unsafe {
      ((&self.parent.sampler_ycbcr_conversion_table).vkDestroySamplerYcbcrConversion)
...
impl<'dev> SamplerYcbcrConversion<'dev> {
  ...
  pub const fn table(&self) -> &SamplerYcbcrConversionDispatchTable {
    &self.parent.sampler_ycbcr_conversion_table
  }
