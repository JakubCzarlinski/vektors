...
impl ShaderInstrumentationARMDispatchTable {
    ...
    F: Fn(*const c_char) -> Option<unsafe extern "system" fn()>,
  {
    Self {
      ...
      #[cfg(feature = "VK_ARM_shader_instrumentation")]
      vkDestroyShaderInstrumentationARM: loader(c"vkDestroyShaderInstrumentationARM".as_ptr())
        .map(|f| unsafe { core::mem::transmute(f) }),
      #[cfg(feature = "VK_ARM_shader_instrumentation")]
      vkGetShaderInstrumentationValuesARM: loader(c"vkGetShaderInstrumentationValuesARM".as_ptr())
        .map(|f| unsafe { core::mem::transmute(f) }),
    }
  }
}
